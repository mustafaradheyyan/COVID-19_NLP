from front_end_gui_script import print_console_gui
import os
import calendar
import pandas as pd
...
def calculate_end_date(start_date, date_format):
...
def check_tweet_availability(keyword, start_date, end_date):
    try:
        for tweet in enumerate(sntwitter.TwitterSearchScraper\
        ...
        print('\nSearch keyword has unsearchable characters, please replace them!\n', flush = True)
        return 0

def tweet_scraper(number_of_tweets_per_month, start_date, end_date, keyword):
    # Using TwitterSearchScraper to scrape data and append tweets to list
    if check_tweet_availability(keyword, start_date, end_date):
        initiate_path()
        final_date = end_date
